...
public class Solution_LCR_86 {
    ...
    /*public String[][] partition(String s) {
        ...
        String[][] finalResult = new String[globalResult.size()][];
        for (int i = 0; i < globalResult.size(); i++) {

            List<String> strings = globalResult.get(i);
            strings.removeFirst();
            finalResult[i] = new String[strings.size()];

            for (int j = 0; j < finalResult[i].length; j++) {
                finalResult[i][j] = strings.get(j);
            }
        }
...
//        List<List<String>> cbbbcc = solutionLcr86.partition("aabba");
        String[][] cbbbcc = solutionLcr86.partition("cbbbcc");
